...
> **公式约定**（与 hopf 对齐、兼顾 Obsidian）：行内用 `\(...\)`，独立用 `\[...\]`；**表格单元格用 Unicode/ASCII**，不写 `$...$`。
...
**Exp1 v2**（`exp1_v2_flux.py`）：作用量加 \(\mu\sum_{i<j<k}\sin^2\Phi_{ijk}\)。结果：flux 惩罚把磁通从随机（~9.4）压到 0，但相位**没有**变实矩阵——它压到了「0 或 π 磁通」（Z₂ 通量态）。铁证：mean(cos Φ) = 1 − 2·pi_frac 精确成立（4 个种子全对），说明每个三角形磁通是二值的（0/π）。
